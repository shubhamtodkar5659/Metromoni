...
<html lang="en">
  ...
  <body>
    ...
    <!-- steeper section   -->
    <section class="signup-step-container  mt50 ">
      <div class="container">
        <div class="row d-flex justify-content-center pt40 shadow"
             style="background-color: white;padding-bottom:3%; border-radius: 10px">
          <div class="col-md-10 ">
            <div class="wizard ">
                ...
                <div class="tab-content"
                  ...
                  <div class="tab-pane active"
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          <label>Candidate Surname <span class="required">*</span></label>
                          <input class="form-control"
                                 type="text"
                                 id="surname"
                                 name="surname"
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input class="form-control"
                                 ...
                                 id="password"
                                 name="password"
                                 placeholder="Enter Password between Length of 6 to 8 Character"
                                 required>
                        </div>
                      ...
                      <div class="col-md-6">
                        <div class="form-group">
                          ...
                          <input class="form-control"
                                 onchange="validateBirthDate(this.value)"
                                 type="date"
                                 name="bDate"
                      ...
                      <div class="col-md-6">
                        ...
                        <div class="form-group  mt-10">
                          <label class="mt-5"
                                 for="female">
                            <span>Female</span>
                            <input class="form-control input_sex"
                                   id="gender_m"
                                   type="radio"
                                   name="gender"
                                   value="Female" />
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <?php
                                                $religionsql = "SELECT * FROM religion WHERE is_active = 1  ";
                                                $religionresult = mysqli_query($conn, $religionsql);
                                                ?>
                                                <select name="religion" class="form-control select" id="religion">
                                                    <option value="">Select Religion</option>
                                                    <?php while ($row_r = mysqli_fetch_array($religionresult)): ?>
                                                        <option value="<?php echo $row_r['name'] ?>"> <?php echo $row_r['name']; ?> </option>
                                                    <?php endwhile; ?>
                                                </select>
                                            </div>
                                        </div>
                  ...
                  <ul class="list-inline pull-right">
                    ...
                    <li><button type="submit"
                              name="u_register"
                              id="cont1"
                              class="default-btn  btn-sm next-step"
                              style="margin-top:0px;">Register</button></li>
                  </ul>
                  <div class="clearfix">
                    <a class="fixed_clearfix "
                       href="login-page.php">
                      <small class=" alreadyaccount">Already have an account ?</small></a>
                  </div>
                </div>
              </form>
            </div>
    ...
    <!-- Flex Nav Script -->
    <script src="js/jquery.flexnav.js"
    ...
    <script>
      function validateFormFields() {
        ...
        if ($('#profile_for').val() == '') {
          ...
          swal({
            title: "Oops...",
            text: "Candidate Surname is required",
            icon: "error",
            buttons: true,
            dangerMode: true,
          })
        ...
        if (this.value.length < 10) {
          ...
          $.ajax({
            ...
            success: function (result) {
              if (result == 1) {
                $("#phone").val('');
                // $("#onchangeerr").html("<span style=color:red;font-weight:400;>Phone number already exist</span>");
                // $("#onchangeerr").trigger("focus");
                swal({
                  title: "Oops...",
                  text: "Phone number already exists in the database",
                  icon: "error",
                  buttons: true,
                  dangerMode: true,
                })
        ...
        if (!re) {
          ...
          $.ajax({
            ...
            success: function (result) {
              if (result == 1) {
                $("#email").val('');
                $("#onchangeerr").html("<span style=color:red;font-weight: 400;>Email address already exist</span>");
                $("#onchangeerr").trigger("focus");
                return false;
              } else if (result == 0) {
                $("#onchangeerr").html("");
  ...
  if (isset($_POST['u_register'])) {
    ...
    $uLang = isset($_POST['lang']) ? $_POST['lang'] : '';
    $uDiet = isset($_POST['diet']) ? $_POST['diet'] : '';
    $uHeight = isset($_POST['height']) ? $_POST['height'] : '';
    $uReligion = isset($_POST['religion']) ? $_POST['religion'] : 'Hindu';
    $uSubCom = isset($_POST['sub-com']) ? $_POST['sub-com'] : '';
    if (isset($_POST['community'])) {
      $communityCheckbox = isset($_POST['community']);
    } else {
      $communityCheckbox = "";
    }
    $HEduSearch = isset($_POST['HEduSearch']) ? $_POST['HEduSearch'] : '';
    $uCollage = isset($_POST['collage']) ? $_POST['collage'] : '';
    $profile_for = isset($_POST['profile_for']) ? $_POST['profile_for'] : '';
    $uProfession = isset($_POST['Prof']) ? $_POST['Prof'] : '';
    $uSpecialization = isset($_POST['Specialization']) ? $_POST['Specialization'] : '';
    $uIncome = isset($_POST['income']) ? $_POST['income'] : '';
    $uLiveWithFamily = isset($_POST['yes/no']) ? $_POST['yes/no'] : '';
    $uBloodGrp = isset($_POST['bGrp']) ? $_POST['bGrp'] : '';
    $uBdate = isset($_POST['bDate']) ? $_POST['bDate'] : '';
    $uAge = isset($_POST['age']) ? $_POST['age'] : '';
    $uGender = isset($_POST['gender']) ? $_POST['gender'] : '';
    $uBtime = isset($_POST['bTime']) ? $_POST['bTime'] : '';
    $uBlocation = isset($_POST['bLocation']) ? $_POST['bLocation'] : '';
    $uBio = isset($_POST['bio']) ? $_POST['bio'] : '';
    $filename = isset($_FILES["userImg"]["name"]);
    $tempname = isset($_FILES["userImg"]["tmp_name"]);
    ...
    // echo $date;
    $check = mysqli_query($conn, "SELECT * FROM `states` WHERE `id` = '$state' AND is_active = 1 ");
    $count = mysqli_num_rows($check);
    if ($count > 0) {
      $sql1 = "INSERT INTO `user_regiter`(`name`,surname,profile_for, `email`, `phone`, `password`, `country`, `state`, `city`, `address`, `marStat`, `lang`, `diet`, `height`, `religion`, `sub-com`, `community-checkbox`, `HighEdu`, `collage`, `prof`, `specialization`, `income`, `yes/no`, `bGrp`, `bDate`, `age`, `gender`, `bTime`, `bLocation`, `filename`,`bio`,`plan_expiry_date`, `status`,type_plan_id,type_plan) 
    VALUES ('$name','$surname','$profile_for','$email','$phone','$password','$country','$state','$city','$address','$marStat','$uLang','$uDiet','$uHeight','$uReligion','$uSubCom','$communityCheckbox','$HEduSearch','$uCollage','$uProfession','$uSpecialization','$uIncome','$uLiveWithFamily','$uBloodGrp','$uBdate','$uAge','$uGender','$uBtime','$uBlocation','$filename','$uBio','$expiry','1',1,'Basic')";
